...
void CreateEnemy(const PxVec3& pos)
{
    ...
    // В PhysX капсула по умолчанию лежит вдоль X. Поворот на 90 градусов вокруг Z делает ось вертикальной Y.
...
void Shoot(const PxTransform& cameraTransform)
{
    ...
    if (!hasHit)
    {
        PrintConsoleLine(L"Выстрел: промах.");
        return;
    }
    ...
    if (enemyHit)
    {
        PrintConsoleLine(L"Выстрел: попадание по противнику.");
        ApplyImpulseToEnemy(dir, GameParams::SHOT_IMPULSE);
    }
    else
    {
        PrintConsoleLine(L"Выстрел: попадание в препятствие.");
    }
}
...
void ExplodeGrenade(int index)
{
    ...
    PrintConsoleLine(L"Граната: взрыв.");
    ...
    {
        PrintConsoleLine(L"Взрыв: противник вне радиуса.");
        return;
    }

    if (IsEnemyProtectedFromExplosion(boomPos, enemyCenter))
    {
        PrintConsoleLine(L"Взрыв: стена закрыла противника, урон не засчитан.");
        return;
    }
    ...
    std::wcout << L"Взрыв: противник получил урон = "
        << damage
        << L", расстояние = "
        << distance
        << L"\n";
...
void UpdateEnemyAI(float dt)
{
    ...
    if (coverFound)
    {
        if (!enemyMovingToCover)
            PrintConsoleLine(L"AI: найдено укрытие, противник перемещается за стену.");
        enemyMovingToCover = true;
    }
    else
    {
        if (enemyMovingToCover)
            PrintConsoleLine(L"AI: укрытие потеряно.");
...
int main()
{
    ...
    PrintConsoleLine(L"Лабораторная работа 2 запущена.");
    PrintHelp();
